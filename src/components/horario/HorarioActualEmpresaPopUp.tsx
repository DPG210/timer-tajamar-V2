    ...
    >
      <div className="bg-white rounded-xl shadow-xl w-full max-w-md p-6">
        <div className="flex items-center justify-between mb-4">
          ...
          <button type="button" onClick={onClose} aria-label="Cerrar" className="text-gray-400 hover:text-gray-600 text-xl leading-none">&times;</button>
        </div>
          ...
          <table className="w-full text-sm">
            ...
            <tbody className="divide-y divide-gray-50">
              {eventos.map((evento, i) => (
                <tr key={i}>
                  <td className="py-2 text-gray-700">{evento.sala}</td>
                  <td className="py-2 text-gray-700">{formatInicio(evento.inicioTimer)}</td>
                  <td className="py-2 text-gray-500">{calcularFin(evento.inicioTimer, evento.duracion)}</td>
                </tr>
              ))}
            </tbody>
          </table>
